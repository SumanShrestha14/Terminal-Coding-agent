...
function resolveInsidePath(path : string ){
  const cwd = process.cwd()
  const resolved = resolve(cwd, path)
  const rel = relative(cwd, resolved)
  if (rel.startsWith("..") || isAbsolute(rel)) {
    throw new Error(`Path is outside of the current working directory: ${path}`)
  }
  return {cwd, resolved}
}
...
export async function executeLocalTools(toolName : string , input : unknown , mode : ModeType) {
    ...
    case "listDirectory":
      {
        ...
        const { cwd , resolved } = resolveInsidePath(path)
    ...
    case "glob":
      {
        ...
        for await (const file of glob.scan({ cwd: resolved, dot: false, onlyFiles: true })) {
          ...
          files.push(relative(cwd, join(resolved, file)))
        }
      ...
      case "bash": {
        ...
        const proc = Bun.spawn(["bash", "-c", command], {
          cwd: resolveInsidePath(".").resolved,
          stdout: "pipe",
          stderr: "pipe",
